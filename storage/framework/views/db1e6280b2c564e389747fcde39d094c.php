...
<?php endif; ?>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div class="flex items-center gap-3 ml-auto">
            <a href="<?php echo e(route('attendancesexport.export')); ?>" class="group">
    ...
    <div class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-wrap items-center gap-6 mb-6 shadow-sm">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex gap-1.5">
                <a href="?year=&status=<?php echo e(request('status')); ?>&major=<?php echo e(request('major')); ?>"
                class="px-3 py-1.5 rounded-lg border text-xs font-bold transition-all
                <?php echo e(!request('year') || request('year') == ''
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex gap-1.5">
                <a href="?status=&year=<?php echo e(request('year')); ?>&major=<?php echo e(request('major')); ?>"
                ...
                <?php $__currentLoopData = ['Good', 'At Risk', 'Critical']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="?status=<?php echo e($status); ?>&year=<?php echo e(request('year')); ?>&major=<?php echo e(request('major')); ?>"
                    class="px-3 py-1.5 rounded-full border text-xs font-bold transition-all
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col gap-3">
            ...
            <div>
                ...
                ?>
                <?php if($activeSemester && $activeSemester->semester_start): ?>
                    <p class="text-sm font-bold text-slate-900 dark:text-white">
                        <?php echo e(\Carbon\Carbon::parse($activeSemester->semester_start)->format('M d, Y')); ?>

                    </p>
                    <p class="text-[10px] text-slate-400 mt-0.5">to</p>
                    <p class="text-sm font-bold text-red-500">
                        <?php echo e($activeSemester->deadline ? \Carbon\Carbon::parse($activeSemester->deadline)->format('M d, Y') : '—'); ?>

                    </p>
                <?php else: ?>
                    <p class="text-sm font-bold text-slate-400">No semester set</p>
                    <a href="<?php echo e(route('attendances.duration')); ?>" class="text-[10px] text-primary font-bold hover:underline mt-1 inline-block">Set now →</a>
                <?php endif; ?>
            </div>
            <?php if($activeSemester && $activeSemester->deadline): ?>
                <?php
                    $daysLeft = \Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($activeSemester->deadline), false);
                ?>
                <p class="text-xs font-medium <?php echo e($daysLeft < 0 ? 'text-red-500' : ($daysLeft <= 14 ? 'text-amber-500' : 'text-violet-600 dark:text-violet-400')); ?>">
                    <?php echo e($daysLeft < 0 ? 'Semester ended' : ($daysLeft == 0 ? 'Ends today' : $daysLeft . ' days remaining')); ?>
...
<?php endif; ?><?php /**PATH D:\Student-Management-System\resources\views/attendances/index.blade.php ENDPATH**/ ?>